...
| A17 | Supply-chain follow-ups (HARDEN-W16): (a) BUMP the fixable HIGH advisories — `drizzle-orm` → ≥0.45.2 (GHSA-gpj5-g38j-94v9, SQL-identifier injection) and `nodemailer` → ≥9.0.1 (GHSA-p6gq-j5cr-w38f, file-read/SSRF) — plus the 8 moderate / 1 low, then raise the CI audit gate from `--audit-level=critical` to `--audit-level=high` (the report step already lists them every run). (b) Enable GitHub **code scanning / Advanced Security** so the CodeQL job's SARIF upload lands and the job can drop `continue-on-error`. (c) Confirm the trufflehog secret-scan behaves on the repo's event types (push/PR) once CI runs live | F+B | before CI is a launch gate | ⬜ open |
...
### C1 · Phase 2 — Optimiser & scale (Q1 27) — **ALL BUILD TASKS ✅ · gate automated-GREEN 2026-07-05**
