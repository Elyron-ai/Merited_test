...
import { createSimulatedTrio } from '@merited/trio/testing';
import pg from 'pg';
import argon2 from 'argon2';
import { AURORA_COMMERCIAL, AURORA_MEMBERS, AURORA_MERCHANT_ID, AURORA_OFFERS, CONTROL_PLANE_ADMIN, } from './fixtures/aurora.js';
const formatPence = (amount) => `£${(amount / 100).toFixed(2)}`;
/**
...
const resetSeedTargets = async (resetDatabaseUrl) => {
...
export const runSeed = async (options) => {
    ...
    try {
        ...
        for (const member of AURORA_MEMBERS) {
            await pool.query(`INSERT INTO core.aurora_club_members (member_ref, sub_hash, loyalty_tier, status)
         VALUES ($1, $2, $3, $4)
         ON CONFLICT (member_ref) DO NOTHING`, [member.member_ref, member.sub_hash, member.loyalty_tier, member.status]);
        }
        ...
        let published = 0;
        let bountyCommitmentId = null;
        for (const fixture of AURORA_OFFERS) {
            const found = await core.repository.get(fixture.offer_id);
            if (!found)
                await core.offers.createDraft(fixture.draft, fixture.offer_id);
            const record = found ?? (await core.repository.get(fixture.offer_id));
            if (record.offer.status === 'draft') {
                const result = await core.publisher.publish(fixture.offer_id, fixture.bounty ? { bounty: fixture.bounty } : undefined);
                published += 1;
                if (fixture.bounty) {
                    bountyCommitmentId = result.commitment_id;
                    log(`Published "${fixture.draft.title}" with a fixed CPA bounty of ` +
                ...
                if (fixture.bounty)
                    bountyCommitmentId = record.current_commitment_id;
        ...
        if (cpUsers.rows[0]?.t) {
            await pool.query(`INSERT INTO control_plane.users (user_id, email, password_hash, totp_secret)
         VALUES ($1, $2, $3, $4)
        ...
        log(`Aurora Experiences ready: ${AURORA_OFFERS.length} offers, ` +
