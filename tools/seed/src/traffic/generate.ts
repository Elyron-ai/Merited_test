import { pence, type Commitment } from '@merited/contracts';
...
export interface TrafficOptions {
  ...
  baseDate: string; // e.g. '2026-07-01'
}
...
export const generateTraffic = async (
  pool: pg.Pool,
  ...
  const id = (prefix: string): string => {
    let body = '';
    for (let i = 0; i < 26; i += 1) body += ULID_ALPHABET[Math.floor(random() * 32)]!;
  ...
  const REASONS = ['BUDGET_EXHAUSTED', 'QUOTE_EXPIRED', 'TOKEN_REPLAYED', 'TIER_INELIGIBLE'] as const;
  let events = 0;

  for (let m = 0; m < merchants; m += 1) {
    const merchantId = id('mer') as `mer_${string}`;
    const commitmentId = id('com') as `com_${string}`;
    ...
    const commitment: Commitment = {
      commitment_id: commitmentId as Commitment['commitment_id'],
      merchant_id: merchantId,
      offer_ref: id('off') as `off_${string}`,
    ...
    for (let d = 0; d < days; d += 1) {
      ...
      const at = (hourFraction: number): string =>
        new Date(dayStart + Math.floor(hourFraction * 86400000)).toISOString().replace(/\.\d{3}Z$/, 'Z');

      for (let k = 0; k < mintsPerDay; k += 1) {
        const jti = id('atk') as `atk_${string}`;
        const qid = id('qte') as `qte_${string}`;
        const aid = agents[Math.floor(random() * agents.length)]! as `agt_${string}`;
        const t = (k + random()) / (mintsPerDay + 1);
        await appendEventInNewTx(pool, 'TokenMinted', {
          claims: {
            jti, cid: commitmentId, qid, aid,
            tier: 'T3' as const, sid: 'f'.repeat(64), apr: null,
...
export const auditExhaust = async (
  pool: pg.Pool,
  minimumRows = 200,
): Promise<{ merchantDays: number; verdict: 'adequate' | 'thin' }> => {
  const { rows } = await pool.query<{ n: string }>(
    'SELECT count(*) AS n FROM core.mint_vs_claim_by_merchant_day',
  );
