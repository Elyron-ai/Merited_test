...
import { UcpCheckoutRail } from '../src/ports/ucp-rail.js';
import { VerdictPoller } from '../src/ports/verdict-poller.js';

/**
 * PH3-3 accept, Valet leg: "Valet's CheckoutRail gains a UCP implementation
 * — an errand completes over the UCP rail." No FakeShop anywhere: the
 * purchase leaves over UCP and the attribution callback arrives at core's
 * UCP intake, signed like any inbound delivery.
 */

const ADMIN = 'postgres://merited_admin:merited_dev@localhost:5432/merited';
const dbName = `merited_ucprail_${Date.now().toString(36)}`;
const SERVICE_TOKEN = 'ucp-rail-test';
...
beforeAll(async () => {
  ...
  driver = new ErrandDriver({
    ...
    resolveSku: async () => 'sku_spa_day', // no shop catalogue on the UCP rail
  });
});
...
describe('errand over the UCP rail (PH3-3 accept)', () => {
  it('BRIEFED → … → CONFIRMED over UCP: token minted at quote, claim verified via the UCP callback', async () => {
