...
**The single most important scheduling event** is **M1 — contract freeze** at the end of Phase 0 week 4: the trio contract shapes + OpenAPI frozen and tagged with senior-dev sign-off, the contract suite green over commitment, mint/verify and settlement posting (TRIO-4…9; the clawback/netting/statement simulators TRIO-10/11 complete behind the frozen shapes in week 5). From that moment the senior developer's ~4-week clock runs independently of the demo date — Phase 0 deliberately ships on simulators (spec §9), and the *unchanged* contract suite is the senior dev's objective acceptance gate at Phase 1.
...
| **PH1** | Phase 1: B6 full, B7, B10, B14, B15, B19, B23, B25, B26, B27 completion, senior-dev trio, partner go-live | 1 | PH1-1…30 | §6 |
...
| SYN-25 | **Phase 0 ships on simulators; the senior dev starts at M1 (end of week 4) and gates at Phase 1.** | Spec §9 wins over architecture §7's "Trio v1" phrasing (spec §0: "when a decision here conflicts…the spec wins"). | XC resolution, ratified. The senior dev starting PH1-24…26 at M1 (Phase 0 week 5) is a *declared* phase-order exception: trio internals sit behind the frozen contract suite and touch nothing Gate 0 measures; Gate 0’s "later-phase modules not started" checkbox carries this exemption explicitly. |
...
| **PH1-8** | **Encrypted refresh-token store** (the `Crypter`/`FakeCrypter` interfaces land in Phase 0 as FND-8 — SYN-16; do not re-declare them). Uses FND-8’s `Crypter`/`FakeCrypter` as-is (the real KMS implementation is PH1-30, senior dev). `link_tokens` table: encrypted refresh tokens keyed by `link_id`, never serialised into contracts or API responses (§3 NB). | wallet migration `link_tokens` (+ store module) | FND-8, PH1-1 | Round-trip encrypt/decrypt property test; ciphertext at rest (raw table read shows no plaintext); PH1-1's lint rule + a log-scrubbing test prove tokens absent from responses and log lines (§6.3 Accept). **Senior-dev review on token storage (B23 note).** | M |
...
The trio remains ★ contract-only for Claude Code (§7). Claude Code's Phase-1 trio work is exactly: PH1-2 (suite extension) and PH1-23 (CI plumbing). PH1-24/25/26/30 are the senior developer's; the plan sequences and gates them but does not decompose their internals.

| ID | Task | Build (exact paths) | Depends on | Accept | Size |
|---|---|---|---|---|---|
| **PH1-23** | **CI dual-target trio contract suite.** (Extends XC-12’s Phase-0 job — same workflow file, not a rebuild; XC-12 built the dual-target parameterisation, this task adds the staging-KMS config and the real-trio demo-e2e variant.) Parameterise the B20 contract suites to run against a target base URL: `TRIO_TARGET=simulator` (default, every PR) and `TRIO_TARGET=real` (senior-dev branch + release pipeline, real KMS via fake-kms locally/staging KMS in CI). One suite, two targets — the suite is the demarcation line made executable. Also: the Phase-0 e2e demo test (§8 testing bar) gains a real-trio variant. | `.github/workflows/trio-contract.yml`, `apps/trio/*/contract-tests/config.ts` | PH1-2, TRIO-13 (B20 suite) | Suite green vs simulator on every PR; the **same** suite runs unmodified against the real trio; any suite edit after senior-dev start requires senior-dev sign-off (the suite is his acceptance gate — §7). | M |
| **PH1-24** | **[SENIOR DEV] Real Commitment Signing service** replaces `apps/trio/commitment/simulator.ts` file-for-file: real Ed25519 via KMS, custodied merchant keys, COR immutability. | `apps/trio/commitment/*` (in-place) | M1 freeze (XC-7) + the frozen TRIO-13 suite (run via `TRIO_TARGET_URL` until PH1-23’s CI job lands); fake-kms → real KMS config | PH1-23 suite green vs real; §5.1 Accept re-verified (bounty edit → second COR; old tokens verify against the first). | SD |
...
| **PH1-26** | **[SENIOR DEV] Real Net Settlement**: replaces `settlement/simulator.ts` file-for-file — the Claude-built `posting.ts` arithmetic and `statements.ts` are **retained** per TRIO-16 (§7.3: "it’s accounting, not crypto"); senior-dev scope is production hardening of transactional posting, counters, clawback reversals and netting runs (weekly cadence; Phase 1 = statements only — Stripe Connect is Phase 2, §2.2). | `apps/trio/settlement/*` (in-place) | PH1-23, PH1-25 | PH1-23 suite green vs real; §7 property test: "trial balance sums to zero after any generated sequence of verify/reverse/net operations." | SD |
| **PH1-30** | **[SENIOR DEV] Real KMS-backed `Signer` + `Crypter`** replacing `FakeSigner`/`FakeCrypter` via config in staging/production: used by agent request-signature verification (PH1-5), mandate/approval attestations (PH1-16/18), custodied claim signing (MER-4) and the `link_tokens` store (PH1-8). Keys via KMS API calls only — never raw keys in process (architecture §6). | `packages/signing/*` (impl), KMS config | PH1-23 (staging config), FND-8 | `fake-`-tagged signatures never appear outside dev/test (env guard); PH1-5/8/16/18 suites green against the real implementations; required before PH1-27 go-live. | SD |
| **PH1-27** | **Design-partner webhook go-live.** Take the B12 Grade-B adapter live with the real partner: merchant onboarded via control plane (real record, commercial config, custodied keypair issued through the **real** commitment service); per-merchant webhook secret + signature verification (even in dev — §8); `Idempotency-Key` honoured; staging dry-run with partner test orders → production cutover checklist; alerting on webhook failures + rejection-reason spikes; partner-facing one-pager (UK English) documenting the one webhook (architecture §5: "one webhook is the minimum-viable merchant ask"). Cutover checklist includes managed-Postgres PITR/backup verification and Postgres RLS belt-and-braces (architecture §6). PH1-20 monitor watching from day one. | `apps/core/src/modules/adapters/<partner>/*` (config, not fork — same Grade-B code path), `apps/control-plane` onboarding flow | MER-3/4/5 (B12), PH1-24, PH1-25, PH1-26, PH1-30, PH1-20, PH1-21 | First real partner order → signed Conversion Claim → **verified by the real trio** → balanced ledger entries → visible in projections; replay of the partner's own webhook returns the original result (idempotency); rejection reasons visible to the partner. **Maps to §9 Ships: "real design-partner webhook live."** | L |
...
- **Lane 4 (trio/senior dev):** the senior dev has been building since Phase 0 week 5 against the M1-frozen suite (SYN-25); PH1-2’s gap-sweep lands in week 1 of Phase 1 and any resulting suite change is senior-dev-signed change control (XC-7); PH1-23 immediately after; PH1-24→26 complete through the quarter with PH1-30 alongside; PH1-27 is the last integration act; PH1-28 closes the phase.
...
**Phase-order rule (spec §0.1).** No Phase 2 module starts while a Phase 1 acceptance test fails; no Phase 3 module while a Phase 2 test fails. **Interpretation:** the long-lead items below (vendor account setup, app-review submission, legal session) are the sanctioned exception and *must* start earlier than their phase. Two carry small builds — LEAD-3’s minimum reviewable app shell and LEAD-4’s synthetic-traffic generator — declared here and capped at the minimum needed to unblock the external clock. The third declared exception is the senior dev’s trio implementations (PH1-24…26), which start at M1 per SYN-25.
...
| PH2-6 | **Stripe Connect payouts behind `PayoutRail`** | `PayoutRail` interface (`createAccount`, `transfer`, `reverse`) already typed in `packages/contracts` with `SimulatedPayouts` as the Ph 1 *behaviour* (statements only — §2.2). Now: `apps/core/src/modules/adapters/payouts/stripe-connect.ts` + a payout worker consuming `SettlementNetted` via `packages/events` ledger reader → one test-mode transfer per net position. Trio stays payout-ignorant (P3: small blast radius; moving funds is a vendor call, not crypto). Contracts-first PR if a payout-execution event is added (open question). | LEAD-1, PH1-24–26 (real settlement + netting run), PH1-29 (SimulatedPayouts + the shared adapter contract test), the contracts-first process (XC.9) | **Phase 2 gate clause:** "netting run produces a real Connect transfer in test mode." Plus: `SimulatedPayouts` and `StripeConnectPayouts` pass the same adapter contract test; `reverse` maps clawback reversals; no live-mode key accepted while LEAD-2 unresolved (env-loader guard). | L |
...
- [ ] Later-phase modules **not started** (§0.1) — save the declared exceptions: the senior dev’s PH1-24…26 from M1 (SYN-25) and the LEAD-* long-leads (§7)
...
| `apps/trio/**` — real PASETO mint/verify, Ed25519/KMS ops, replay-store hardening, production posting hardening (the pure `posting.ts` arithmetic is Claude-built and retained — spec §7.3, TRIO-16) | **Senior dev** — Claude Code must not implement | n/a (SD-authored) | File-for-file replacement of simulators; acceptance = XC-12 |
...
| Q5 | **Hosting** — Fly.io/Render vs AWS, deliberately deferred (XC D4); forced by the partner security review at Phase 1 hardening, before PH1-27 cutover. The decision carries architecture §6’s data requirements — managed Postgres with PITR from day one of hosting ("backups are existential") and Postgres RLS as tenancy belt-and-braces — both on PH1-27’s cutover checklist. | Founder + senior dev | Phase 1, before PH1-27 |
