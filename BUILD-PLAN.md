...
**Audience:** the primary builder working with Claude Code, and the senior developer who owns the security-critical trio.
...
4. **The trio boundary is absolute** (BUILD-SPEC §7). Claude Code builds contracts, simulators and test suites in `apps/trio`; real PASETO/Ed25519/KMS/production-posting work is the senior developer's, replacing simulators file-for-file. §8's ownership table (XC.7) makes the boundary mechanical via CODEOWNERS.
...
| **1 — Harden & first partners** | Q4 26 | Real trio lands; design partner live | Contract suite green against the **real** trio, unchanged; OAuth linking round-trip in CI; walletless-T1 proven; mint-vs-claim monitor live |
...
**Phase 0 is planned at task resolution** across five workstreams — FND (foundations: monorepo, contracts, event ledger, signing fakes, observability, CI), TRIO (trio contracts + behaviourally faithful simulators + the contract test suite that later gates the senior dev), CORE (the canonical `readOffers()` path: offers, agents, identity, eligibility, quotes, REST + SDK), MER (Grade-B webhook adapter, thin control plane, FakeShop storefront), VAL (Valet v0, seed tooling, the Act 1 demo that doubles as the repo's single E2E CI test) — plus XC (cross-cutting: conventions, testing strategy, gates, risks, ownership). Phase 1 is planned at task resolution (PH1-1…28); Phases 2–3 at milestone resolution (PH2-*/PH3-* with long-lead items LEAD-1…4 that start earlier than their phase).

**The single most important scheduling event** is **M1 — contract freeze** at the end of Phase 0 week 4: trio contracts + all three simulators + the full contract suite green, senior-dev sign-off on the trio-touching schemas. From that moment the senior developer's ~4-week clock runs independently of the demo date — Phase 0 deliberately ships on simulators (spec §9), and the *unchanged* contract suite is the senior dev's objective acceptance gate at Phase 1.
...
| **TRIO** | Trio contracts, simulators, contract test suite (B20, spec §7) | 0 (+PH1-2/17/22…26 in Phase 1) | TRIO-1…17 | §5.2 |
...
| **PH1** | Phase 1: B6 full, B7, B10, B14, B15, B19, B23, B25, B26, B27 completion, senior-dev trio, partner go-live | 1 | PH1-1…28 | §6 |
...
| "TRIO-R1/R2/R3", "senior-dev real trio", "TRIO-9 (real impls)" | PH1-24 (commitment) / PH1-25 (mint+verify) / PH1-26 (settlement) |
...
|---|---|---|---|
| SYN-1 | **`packages/signing` is built once, by FND-8.** | TRIO-2 is subsumed — do not build twice. TRIO contributes the key-hierarchy registry requirement (platform mint / per-merchant custodied / per-agent key refs, namespaced so cross-hierarchy signatures fail verification) and reviews FND-8 before simulator work begins. | Interface churn here is the most expensive kind; senior-dev review of FND-8 is a merge blocker. |
| SYN-2 | **`packages/otel` is an approved deviation from the §1 layout.** | Ratified (FND D6). Contracts stays types-only; every app boots via `@merited/otel/register`. | Smallest possible deviation; relocatable while nothing depends on the path. |
...
| SYN-21 | **Valet's ledger mirror: direct append via `@merited/events` behind a `LedgerMirror` port in Phase 0.** | §6.6 bans private *Core* imports, not shared packages. The port isolates a possible Phase-2 switch to a public mirror endpoint (P5 purism) — decide at Phase 2 hardening. | VAL D3. |
| SYN-22 | **Merchant keypair issuance is a trio-adjacent contract: `POST /trio/keys/merchant`** (MER-2), simulator-implemented, inherited by the senior dev via TRIO-16's HANDOFF. | Key-generation authority stays inside the trio's blast radius (P3). | |
...
| SYN-24 | **Auth conventions (Phase 0).** Agents: `X-Merited-Agent-Key` — absent header = anonymous degraded read, present-but-invalid = 401; anonymous rate-limited by IP. Merchants: `X-Merited-Merchant-Key` on `/v1/claims`; HMAC (`X-Merited-Signature` + timestamp, ≤300s skew) on webhooks, enforced in every environment. Control plane: credential + TOTP. Core→trio: shared-secret header locally; mTLS/service tokens are senior-dev hardening (architecture §6). | | |
| SYN-25 | **Phase 0 ships on simulators; the senior dev starts at M1 (end of week 4) and gates at Phase 1.** | Spec §9 wins over architecture §7's "Trio v1" phrasing (spec §0: "when a decision here conflicts…the spec wins"). | XC resolution, ratified. |
| SYN-26 | **`check_eligibility` (MCP) mints no tokens.** | P2's "every offer read mints" applies to responses containing payable offers; `check_eligibility` returns verdicts + exclusion reasons only. `search_offers`/`get_offer` return tokenised quotes. | PH1-6. |
| SYN-27 | **Exclusion reasons are a separate enum from the 12 claim-rejection codes.** | `ExclusionReason` (e.g. `STACKING_SUPPRESSED`, `MERCHANT_EXCLUDED`, `TIER_INELIGIBLE`, `OFFER_NOT_LIVE`) added in contracts for eligibility's `excluded[{offer, reason}]`; the §3 codes stay claim-only. Where Phase 0 CORE-6 text reuses §3 code names for exclusions, read them as the `ExclusionReason` equivalents. | PH1-1 lands the enum; CORE-6 adopts it. |
...
Phase 0 at task-group level; Phases 1–3 at milestone level. `M1 CONTRACT FREEZE` is the moment the senior dev's clock starts.
...
flowchart TD
    ...
    SD["PH1-24/25/26 senior dev:<br/>real trio impls (~4 wks)"]
    ...
    subgraph LATER["PHASES 1–3 — milestones"]
        ...
        GLIVE["PH1-27 design-partner go-live"]
...
**Critical path through Phase 0** (each item blocks the next):
...
**Parallel lanes for a single primary builder.** With one builder, "parallel" means interleaving: whenever the critical-path task is blocked on CI, review, or thought, pull from the filler lanes. Lanes that are genuinely independent once FND-3/4 land: (a) trio contracts + simulators; (b) the CORE-1/2/3/4/6 module lane; (c) the low-stakes lane — MER-7…10 control plane, FND-14 OTel, MER-11 FakeShop, VAL-9 seed, VAL-11 harness, and all XC tasks. The only true concurrency arrives at **M1 CONTRACT FREEZE**, when the senior dev starts PH1-24…26 against the frozen contracts while the builder continues Phase 0 above the simulators. That is the entire point of §7's "behaviourally faithful simulators": the senior dev's start date is decoupled from the demo's ship date.
...
|---|---|---|---|
| 1 | FND-1 scaffold + FND-2 compose; FND-3 contracts primitives; FND-4 §3 objects; FND-5 mechanics union started | XC-1 conventions, FND-16 CI skeleton, XC-4 ADR log — **week-1 decisions D1–D8 (§8, XC.8) made and recorded**; FND-6 ports | CI green on the empty-but-compiling workspace; `docker compose up` clean; contracts parse all §3 examples |
| 2 | FND-7 event schemas + catalogue; FND-9 migrations; FND-10 append core + hash chain; FND-11 outbox; FND-13 verify-chain; FND-8 signing fakes | CORE-1 skeleton, CORE-2 offers storage, CORE-3 agent registry started; FND-12 projections; XC-5 test kit | B2 accept: chain verifies, tamper detected, REVOKE proven; B3/B4 schemas round-tripping |
| 3 | TRIO-1 trio contracts (senior-dev review async); TRIO-3 scaffold; TRIO-4 commitment sim; TRIO-5 mint sim + TRIO-6 replay store | CORE-4 identity resolver, CORE-6 eligibility-min; MER-1 contracts + MER-2 merchants module; XC-6 property harness | Mint→verify happy path shapes green vs sims; identity precedence property tests green |
| 4 | TRIO-7 directory port; TRIO-8 verify pipeline (all 12 reason codes); TRIO-9 posting; TRIO-12 trial-balance property; TRIO-13 suite consolidation; **XC-7 M1 CONTRACT FREEZE at week end** — tag `@merited/contracts@0.1.0` + trio OpenAPI, senior-dev sign-off | CORE-2/3 finished to their Accepts; CORE-7 stubs + CORE-8 pricing; XC-3 CODEOWNERS live | Full contract suite green vs all three sims; trial balance zero; **senior dev starts PH1-24 in week 5** |
| 5 | CORE-9 token-client; CORE-10 quote service; CORE-11 pipeline assembly; CORE-12 REST surface | FND-14 OTel end-to-end trace; VAL-9/10 seed; TRIO-10 clawback + TRIO-11 netting/statements; TRIO-14 OpenAPI | B24 accept: every payable read persists exactly one quote; B4 accept: registered vs unregistered read behaviour |
...
| 7 | VAL-5 ports; VAL-6 driver; VAL-7 CLI; VAL-12 demo Act 1 steps 1–7 (grown incrementally behind the VAL-14 CI skeleton) | VAL-8 durability; VAL-11 harness (if not earlier); MER-10 claims viewer; MER-12 e2e joined; XC-8 demo-as-E2E job | `pnpm demo:act1` steps 1–7 end-to-end locally; reducer transition tests exhaustive |
| 8 | VAL-13 negatives + trace/chain proof; VAL-14 E2E in CI; VAL-15 clean-machine bootstrap; **XC-9 `pnpm gate:0` green; record the demo** | Buffer — absorb overruns; TRIO-16 handoff pack review; risk-register review; BUILD-PLAN status sweep | **GATE 0 checklist (§8, XC.5) fully ticked; demo asset recorded** |
|  | *Senior-dev track, weeks 5–8+:* PH1-24 (real commitment signing) → PH1-25 (PASETO mint + verification hardening) → PH1-26 (settlement), replacing `apps/trio/*/simulator.ts` file-for-file; acceptance = the **unchanged** TRIO-13 suite via XC-12 (`TRIO_TARGET=real`). Real trio lands as M2 and gates at Phase 1 per §9 — the Phase 0 demo deliberately ships on simulators (SYN-25). Work extending past week 8 continues into Q4 without blocking Gate 0. | | |
...
| D7 | **fake-kms image** | `nsmithuk/local-kms` in docker-compose. The in-process `FakeSigner`/`FakeCrypter` do **not** depend on it (unit tests must run docker-free); the container exists so the senior dev's real KMS-backed `Signer` has a local target, and for compose parity with prod. |
...
**Boundary (spec §0.3, §7, architecture P3).** Claude Code builds: TypeScript interfaces and Zod schemas in `packages/contracts`, the trio HTTP surface and OpenAPI docs, three behaviourally faithful simulators, the Postgres unique-`jti` replay logic (explicitly in scope — "build this properly, it's not crypto"), the real double-entry settlement arithmetic (explicitly in scope — "it's accounting, not crypto"), and the exhaustive contract test suite that later gates the senior dev's real implementations. Claude Code must **not** build: real PASETO v4.public mint/verify, Ed25519/KMS key operations, replay-store hardening (Redis fast-path, adversarial tuning), or production ledger-posting hardening. The senior dev replaces simulators **file-for-file** at `apps/trio/*/simulator.ts`; everything else in `apps/trio` (routes, DB schema, replay store, posting arithmetic, tests) is retained.
...
packages/signing/                  ← Signer interface + FakeSigner (deterministic HMAC pseudo-sigs)
apps/trio/
...
| `COMMITMENT_ENDED` | 5 | `POST /trio/commitments/:id/end`, then claim old token *(also proves §5.1: tokens minted under the **first** COR still verify — ending applies per-commitment)* | — |
...
| Phase 1: "contract suite green against **real** trio" | TRIO-13 (run by senior dev, zero edits) |
...
|---|---|---|---|---|
| CORE-1 | Core app skeleton: Fastify boot, module tree, Drizzle, RateLimiter adapter+fake | §1, §2.2, §8 | FND-3/4/5, FND-1/2, FND-14 | M |
| CORE-2 | Offers storage + CRUD service: `offers`, `offer_counters`, COR history link | B3, §5.1 | CORE-1 | M |
...
| CORE-7 | Decisioning Slot + Guardrails stubs: interfaces in contracts, Passthrough/Noop impls | B7/B8, §5.5/§5.6 | CORE-1, FND-3/4/5 | S |
| CORE-8 | `applyMechanics` pure pricing function in `packages/contracts` | B24, §5.6a | FND-3/4/5 | S |
...
3. Commitment liveness + cap: batched query to the trio's commitment-status read endpoint (simulator in Ph0) → exclude `COMMITMENT_ENDED` / `CAP_EXHAUSTED`. **Dependency flag:** §7.1 lists no read endpoint; TRIO-1/4 must add `GET /trio/commitments/:id` → `{status, conversions_used, max_conversions}` (resolved — SYN-7: TRIO-4 provides it).
...
| MER-2 | Merchants module: CRUD, webhook secrets, custodied-key issuance | M | MER-1, FND-7/10/13, FND-8, TRIO-1/4 | A |
...
| MER-5 | Claims API: `POST /v1/claims` (merchant auth) + `GET /v1/claims/:id` | S | MER-4 | A |
...
| MER-11 | FakeShop storefront in `apps/fake-aurora` | M | MER-1, MER-3 (URL/signature contract only) | C (parallel to A/B) |
...
`adapters/grade-b/normalise.ts`: native payload → `OrderConfirmed` — `sha256` the raw order ref (basket contents dropped at this boundary; only hash + money numbers persist, per architecture §2.4 data minimisation), amounts validated as integer pence. `adapters/grade-b/claim-builder.ts`: `OrderConfirmed` → `ConversionClaim {clm_…}`; `merchant_sig` produced by `Signer.sign(signing_key_ref, canonical_json(claim))` — custodied key, `FakeSigner` in Phase 0, **no cryptography implemented in this workstream** (§7 boundary: MER calls the `Signer` interface and the trio's HTTP contract as black boxes). Emit `ConversionClaimed` via the FND-7/10/13 outbox, then `POST /trio/claims/verify`; persist claim row + verdict + `reason_code` in `claims` table. Missing/absent token in the order → claim still built with `token: null` and submitted; trio rejects `SIG_INVALID`/token-absent per its contract — the rejection is data (spec §3: "both sides must see why").
...
Phase 1 turns the Phase 0 proof point into a platform a design partner can run against: full deterministic eligibility, the pluggable Decisioning Slot, the MCP agent surface, the consent/identity layer (mandates, account linking, FakeAurora IdP, push + approvals), the mint-vs-claim monitor, audit-trail hardening, and — the phase's defining event — the senior developer's real trio implementations replacing the simulators file-for-file with the existing contract suite as the acceptance gate.
...
Ships (§9): *B6 full, B7, B10, B14, B15, B19, B23 (linking + FakeAurora IdP), B25 (push), B26 (approvals); senior-dev trio implementations land; real design-partner webhook live.*
...
| **PH1-8** | **Encrypted refresh-token store** (the `Crypter`/`FakeCrypter` interfaces land in Phase 0 as FND-8 — SYN-16; do not re-declare them). `Signer`-adjacent `Crypter` interface (§6.2) in `packages/signing` (encrypt/decrypt via KMS data key; **real impl senior dev**); `FakeCrypter` (deterministic AEAD over a dev key) for local/CI. `link_tokens` table: encrypted refresh tokens keyed by `link_id`, never serialised into contracts or API responses (§3 NB). | `packages/signing/src/crypter.ts`, `packages/signing/src/fake-crypter.ts`, wallet migration `link_tokens` | FND-8, PH1-1 | Round-trip encrypt/decrypt property test; ciphertext at rest (raw table read shows no plaintext); PH1-1's lint rule + a log-scrubbing test prove tokens absent from responses and log lines (§6.3 Accept). **Senior-dev review on token storage (B23 note).** | M |
...
| **PH1-20** | **Mint-vs-claim monitor live.** On top of `mint_vs_claim_by_merchant_day`: per-merchant claim-rate computation, configurable floor + trailing window, alert emission (OTel metric + structured alert log → Axiom/Grafana per §2.2) and a control-plane badge on the merchant record (architecture §5: the under-reporting failure mode "deserves a line in the risk register"). Scheduled to run continuously against partner traffic. | `apps/core/src/modules/analytics/mint-vs-claim-monitor.ts`, `apps/control-plane/src/app/merchants/[id]/health/*` | PH1-19, MER-3/4/5 (B12) | **Maps to gate: "mint-vs-claim monitor live."** Synthetic test: mint N tokens, claim < floor% → alert fires within one projection cycle; healthy merchant → no alert; monitor visibly running against the design partner before PH1-27 sign-off. | M |
| **PH1-21** | **Audit-trail hardening (1/2) — hash-head publication.** Daily job publishing the ledger head hash externally (architecture §2.6: "simplest credible option: a public S3 object + posted to the merchant's own systems; blockchain anchoring is unnecessary theatre"). Build an `ObjectStore` adapter (put/get, S3-backed) + `FakeObjectStore` (local filesystem) per the §2.2 adapter+fake rule; `HeadPublication` record from PH1-1; extend the B2 `verify-chain` CLI to cross-check the chain against published heads; optional per-merchant webhook posting the head. This is the concrete foundation for Phase 3's third-party verification — publish format is part of the future open spec, so keep it dumb and stable (JSON: `{date, seq, head_hash}`). **Senior-dev review (trio-adjacent).** | `packages/events/src/{head-publisher.ts,object-store.ts,fake-object-store.ts}`, `packages/events/src/cli/verify-chain.ts` (extend), scheduled job in `apps/core` | FND-7–13 (B2 + verify-chain), PH1-1 | `verify-chain --against-heads <store>` passes on an intact ledger and **fails loudly** on a single mutated historic row; publication job idempotent per day; FakeObjectStore CI test + real S3 smoke test at go-live. | M |
| **PH1-22** | **Audit-trail hardening (2/2) — key-rotation runbook + rotation-tolerant tests.** Documented rotation procedure for the three key hierarchies (platform mint keys, per-merchant keys, per-agent keys — architecture §6 "Keys"; §7 Ph 1 work item). Claude Code writes the runbook skeleton, key-id conventions in signatures/tokens so old artefacts verify after rotation, and simulator-level tests that tokens/CORs minted under key N verify after rotation to N+1. **KMS specifics and the rehearsal are senior-dev scope**; Claude Code must not touch real key handling (§7). | `apps/trio/runbooks/key-rotation.md`, key-id conventions in `packages/signing` + `apps/trio/*/contract-tests` | FND-8, PH1-2, PH1-25 (rehearsal step only) | Rotation-tolerance contract test green against simulators; runbook reviewed + rehearsed by senior dev against the real trio before PH1-27; runbook includes revocation/compromise path per hierarchy. | S |

##### Track D — Real trio landing & design-partner go-live

The trio remains ★ contract-only for Claude Code (§7). Claude Code's Phase-1 trio work is exactly: PH1-2 (suite extension) and PH1-23 (CI plumbing). PH1-24/25/26 are the senior developer's; the plan sequences and gates them but does not decompose their internals.

| ID | Task | Build (exact paths) | Depends on | Accept | Size |
|---|---|---|---|---|---|
| **PH1-23** | **CI dual-target trio contract suite.** Parameterise the B20 contract suites to run against a target base URL: `TRIO_TARGET=simulator` (default, every PR) and `TRIO_TARGET=real` (senior-dev branch + release pipeline, real KMS via fake-kms locally/staging KMS in CI). One suite, two targets — the suite is the demarcation line made executable. Also: the Phase-0 e2e demo test (§8 testing bar) gains a real-trio variant. | `.github/workflows/trio-contract.yml`, `apps/trio/*/contract-tests/config.ts` | PH1-2, TRIO-13 (B20 suite) | Suite green vs simulator on every PR; the **same** suite runs unmodified against the real trio; any suite edit after senior-dev start requires senior-dev sign-off (the suite is his acceptance gate — §7). | M |
| **PH1-24** | **[SENIOR DEV] Real Commitment Signing service** replaces `apps/trio/commitment/simulator.ts` file-for-file: real Ed25519 via KMS, custodied merchant keys, COR immutability. | `apps/trio/commitment/*` (in-place) | PH1-23, fake-kms → real KMS config | PH1-23 suite green vs real; §5.1 Accept re-verified (bounty edit → second COR; old tokens verify against the first). | SD |
| **PH1-25** | **[SENIOR DEV] Real Token Mint + Conversion Verification**: PASETO v4.public minting/verification, KMS key ops, hardened replay store (Postgres unique on `jti` authoritative, Redis fast-path), full §7.2 check order incl. quote-liveness and approval/limit checks. | `apps/trio/verification/*` (in-place) | PH1-23, PH1-2 | PH1-23 suite green vs real incl. all PH1-2 approval/limit/re-mint cases; demo negative cases (`TOKEN_REPLAYED`, `QUOTE_EXPIRED`) reproduce against real tokens. | SD |
| **PH1-26** | **[SENIOR DEV] Real Net Settlement**: production double-entry posting, clawback reversals, netting runs (weekly cadence, Phase 1 = simulated payouts/statements only — Stripe Connect is Phase 2, §2.2). | `apps/trio/settlement/*` (in-place) | PH1-23, PH1-25 | PH1-23 suite green vs real; §7 property test: "trial balance sums to zero after any generated sequence of verify/reverse/net operations." | SD |
| **PH1-27** | **Design-partner webhook go-live.** Take the B12 Grade-B adapter live with the real partner: merchant onboarded via control plane (real record, commercial config, custodied keypair issued through the **real** commitment service); per-merchant webhook secret + signature verification (even in dev — §8); `Idempotency-Key` honoured; staging dry-run with partner test orders → production cutover checklist; alerting on webhook failures + rejection-reason spikes; partner-facing one-pager (UK English) documenting the one webhook (architecture §5: "one webhook is the minimum-viable merchant ask"). PH1-20 monitor watching from day one. | `apps/core/src/modules/adapters/<partner>/*` (config, not fork — same Grade-B code path), `apps/control-plane` onboarding flow | MER-3/4/5 (B12), PH1-24, PH1-25, PH1-26, PH1-20, PH1-21 | First real partner order → signed Conversion Claim → **verified by the real trio** → balanced ledger entries → visible in projections; replay of the partner's own webhook returns the original result (idempotency); rejection reasons visible to the partner. **Maps to §9 Ships: "real design-partner webhook live."** | L |
...
| §9 Ships: senior-dev trio implementations land | PH1-24, PH1-25, PH1-26 | Simulators replaced file-for-file; suite green |
| §9 Ships: real design-partner webhook live | PH1-27 | First verified real-partner conversion in the ledger |
...
- **Lane 4 (trio/senior dev):** PH1-2 lands in week 1 so the senior dev starts against a **complete** acceptance suite; PH1-23 immediately after; PH1-24→26 run through the quarter; PH1-27 is the last integration act; PH1-28 closes the phase.

Critical path: **PH1-1 → PH1-2 → PH1-23 → PH1-25 → PH1-26 → PH1-27 → PH1-28** (senior-dev throughput bound). Longest Claude-Code chain: PH1-1 → PH1-9 → PH1-13 → PH1-16 → PH1-18.

##### Senior-dev review register (beyond the trio itself)

Per architecture §7 ("senior-dev review on trio-adjacent code and OAuth token storage"): PH1-5 (request-signature canonicalisation/verification), PH1-8 + PH1-13 (refresh-token storage, `Crypter` usage), PH1-18 (re-mint call path into the mint), PH1-21 (head-publication format + verify-chain changes), PH1-22 (runbook + rehearsal). Schedule these reviews as merge blockers, not post-hoc.
...
| PH2-12 | **Phase 2 gate run + recording** | Execute all three §9 gate clauses on a clean machine; record Act 2 with `VALET_DETERMINISTIC=1`; file evidence (trace URLs, CI links, Connect test-mode transfer ID) in `docs/gates/phase-2.md`. | PH2-6, PH2-7, PH2-11 | All three clauses green (see mapping below). Phase 3 module work may not start before this passes (§0.1). | S |
...
| Milestone | Target | Contents |
|---|---|---|
| P3.M1 — Open surfaces | end Apr 27 | PH3-1 JSON-LD, PH3-9 SKU granularity |
| P3.M2 — Protocol interop | mid May 27 | PH3-2 harness/mapping, PH3-3 UCP, PH3-4 ACP |
| P3.M3 — Merchant scale | end May 27 | PH3-5 Shopify Grade A, PH3-6 self-serve onboarding |
...
This section is the connective tissue of the plan: the testing strategy, the phase-gate definitions of done, the risk register, the Claude Code / senior-dev ownership boundary, and the repo conventions that keep all of it honest. (The dependency graph and Phase 0 calendar drafted for this section live in §4.) Task references use the authoritative IDs per §2's key.
...
|---|---|---|---|---|---|
| XC-1 | Repo conventions pack: CONTRIBUTING.md (branch naming `feat/<TASK-ID>-slug`, UK English + integer-pence rules, contracts-first rule), PR template with mandatory Task-ID field and "touches contracts first?" checkbox | `CONTRIBUTING.md`, `.github/PULL_REQUEST_TEMPLATE.md` | FND-1 | PR template renders on a test PR; CONTRIBUTING cites §1 contracts rule verbatim | S |
| XC-2 | CI pipeline: lint → typecheck → per-package Vitest → integration (docker-compose services) → contract-suite job; lint rules incl. `no-schema-outside-contracts` (Zod schema declarations only in `packages/contracts`) and no-float-currency | `.github/workflows/ci.yml`, `.eslintrc` custom rules | FND-1, XC-1 | Failing unit test reds the build; a `z.object` added under `apps/core` fails lint | M |
| XC-3 | CODEOWNERS + branch protection encoding the review-required zones (table XC.7): trio, signing, token-client, adapter claim-signing, ledger DDL, linking token store | `.github/CODEOWNERS` | XC-1 | PR touching `apps/trio/**` cannot merge without senior-dev review | S |
...
| XC-7 | **M1 Contract-freeze process**: freeze checklist (trio suite green vs sims; senior-dev sign-off on Commitment/token-claims/ConversionClaim/Approval shapes; D1/D6 ratified), tag `@merited/contracts@0.1.0` + trio OpenAPI 0.1.0, post-freeze change control (breaking contract change = contracts-first PR + suite update in same PR + senior-dev approval + minor-version bump) | `docs/decisions/ADR-009-contract-freeze.md`, git tags | FND-3–5, TRIO-1/14, TRIO-13 | Tag exists; CI pins suite to tagged contracts; senior-dev kickoff brief issued | S |
...
| XC-12 | Trio acceptance gate CI job: contract suite parameterised `TRIO_TARGET=simulator\|real`; Phase 0 runs simulator; Phase 1 gate flips to real — this job **is** the senior dev's acceptance gate (§7 Accept) | `.github/workflows/ci.yml` (job `trio-contract-suite`) | TRIO-13, XC-2, XC-7 | Same suite binary green vs sims (Ph 0) and vs PH1-24–26 (Ph 1 gate) with zero test edits | S |
| XC-13 | Phase 1 hardening review pack: OAuth token-storage review checklist (per §9 Ph 1), refresh-token-leak lint rule + test (B23), key-rotation runbook review item, hash-head publication check, execute deferred hosting decision (D-4) | `docs/reviews/ph1-hardening.md`, lint rule in XC-2 config | XC-3, PH1-13, PH1-25 | Senior-dev sign-off recorded on token storage; lint fails if `refresh_token` appears in any contract type, API response fixture, or log serialiser | M |

XC-1..6 belong in week 1–3; XC-7 fires at end of week 4; XC-8/9 in weeks 7–8; XC-13 is Phase 1.
...
3. **Trio contract suite as the senior-dev acceptance gate** (§7 Accept). TRIO-13 runs target-agnostic over HTTP against `TRIO_TARGET`. Phase 0: green vs simulators. Phase 1 gate: the **identical, unedited** suite green vs PH1-24–26 (XC-12). Any test edit required to pass against the real implementation is by definition a contract bug and reopens the freeze (XC-7 change control).
...
- [ ] Later-phase modules **not started** (§0.1 phase-order rule)
...
- [ ] Real design-partner webhook live
- [ ] Senior-dev review sign-offs recorded: trio-adjacent code, OAuth token storage (§9 Ph 1)
...
Likelihood/Impact: L/M/H. "Builder" = primary builder; "SD" = senior dev; "Founder" = commercial/programme owner.
...
| R2 | **Design partner rejects custodied merchant keys** (arch §8 Q3) | M | H | Raise in the partner's security review *before* Phase 1 wire-up; API already assumes merchant-held keys (arch §2.2) so key-handover pulls forward without contract changes | Founder + SD |
| R3 | **Senior-dev availability vs the ~4-week trio budget** | M | H | M1 freeze decouples SD start date from demo date; simulators keep everything above testable; unchanged contract suite (XC-12) makes the acceptance gate objective; trio scope strictly §7 — no scope creep past the boundary | Founder (schedule), Builder (sim fidelity) |
...
| R8 | **OAuth token-storage weakness** leaks refresh tokens | L | H | B23 design: encrypted at rest via `Crypter`/KMS data key, separate table, never in contracts; mandatory SD review before Gate 1 (XC-13); leak lint + test in CI | SD (review), Builder (impl) |
...
#### XC.7 Ownership boundary — Claude Code vs senior dev

Enforced mechanically by CODEOWNERS + branch protection (XC-3). "SD review required" = cannot merge without senior-dev approval.

| Area / paths | Built by | Review | Notes |
|---|---|---|---|
| `packages/contracts/**` | Claude Code | SD review on trio-touching schemas (Commitment, AttributionTokenClaims, ConversionClaim, Approval attestation) at M1 and on any post-freeze change | Single source of types (§1) |
| `apps/trio/**` — HTTP contracts, Zod, OpenAPI, `simulator.ts` × 3, contract test suite | Claude Code | SD review before M1 freeze | §7: contract-only zone for Claude Code |
| `apps/trio/**` — real PASETO mint/verify, Ed25519/KMS ops, replay-store hardening, production double-entry posting | **Senior dev** — Claude Code must not implement | n/a (SD-authored) | File-for-file replacement of simulators; acceptance = XC-12 |
| `packages/signing/**` — interface + `FakeSigner` | Claude Code | SD review | Real `Signer` impl: senior dev |
| `apps/core/src/modules/token-client/**` | Claude Code | **SD review required** | Trio-adjacent (arch §7 Ph 1 note) |
| `apps/core/src/modules/adapters/**` — claim construction + custodied-key signing calls | Claude Code | **SD review required** | Touches merchant key custody (R2) |
| `packages/events/**` — ledger DDL, `REVOKE UPDATE, DELETE` migrations, hash-chain + `verify-chain` | Claude Code | **SD review required** | The ledger is the product (P1) |
| `apps/wallet/src/modules/linking/**` + `link_tokens` table + `Crypter` | Claude Code | **SD review required** (§9 Ph 1 gate item) | OAuth token storage (R8, XC-13) |
| `apps/wallet/src/modules/mandates/**` — attestation path | Claude Code | SD review | Arch closing note: §4.1 is review surface |
...
| D3 | **Quote TTL default + token TTL relationship** | Confirm 15 min quote default (§3 `OfferQuote`); token `exp` = min(attribution window, quote-compatible TTL) with invariant `quote.expires_at ≤ token exp` asserted in contracts | Baked into B24, TRIO-5/6/8, and both demos |
...
| D6 | **PASETO library + fake-token format** | Pick the real-impl library now (e.g. `paseto` for Node) so TRIO-1/14's token contract and TRIO-5/6/8's base64-JSON pseudo-token share exact claim shapes; SD ratifies at M1 | The senior dev inherits the choice; contract test vectors depend on it |
...
- **PR discipline.** Every PR names exactly one primary task ID from BUILD-PLAN.md in its title; the PR description quotes the task's Accept clause and says how the diff meets it. Review-zone paths (XC.7) auto-request the senior dev via CODEOWNERS.
...
| Q2 | **Phase 0 calendar** — confirm the w/c 6 Jul 2026 start and the senior dev's week-5 start; if the senior dev can only start earlier/later, the M1 freeze scope flexes (see §8, XC.7 note). | Founder + senior dev | Week 1 |
| Q3 | **Custodied merchant keys** — confirm the design partner's security team accepts custodied keys for the pilot (architecture §8 Q3), or pull key-handover forward. | Founder + senior dev | Before senior-dev week 1 (M1) |
...
| Q5 | **Hosting** — Fly.io/Render vs AWS, deliberately deferred (XC D4); forced by the partner security review at Phase 1 hardening, before PH1-27 cutover. | Founder + senior dev | Phase 1, before PH1-27 |
...
| Q9 | **Valet ledger-mirror transport** — keep direct `@merited/events` append (SYN-21) or switch to a public mirror endpoint. | Builder + senior dev | Phase 2 hardening |
...
| Q11 | **Conversion proof-pack composition** for third-party verification (PH3-7) — merchant-held claim + event slice vs Merited-published inclusion proofs. | Builder + senior dev | Phase 3 (earlier if UIP moves — R9) |
